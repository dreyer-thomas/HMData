    ...
    return new Promise((resolve,reject) => {
        ...
        stmt = db.get("SELECT CHANNELID, NAME, DEVICEID FROM CHANNELS where ROWID=?",channelID, (err,rows) => {
            if (err) {
                ...
                stmt2 = db.all("SELECT VALUE, TIME FROM ACTUAL_VALUES WHERE CHANNEL=? AND MEASUREMENT=? AND TIME >= ? AND TIME <? ORDER BY TIME",channel, measurement, tstart, tend, (err2,datapoints) => {
                    if (err2) {
                        console.log(err2);
